	...
	renderPosts() {
		if (_.keys(this.props.posts).length === 0) {
			if (this.props.match.params.category) {
				return (
					<li className="post list-group-item" key="Not found">
						<h5>Could not find any post for this category.</h5>
					</li>
				);
			} else {
				return (
					<li className="post list-group-item" key="Loading">
						<h5>Loading...</h5>
					</li>
				);
			}
				...
				<div className="panel-body">
					<div className="pull-left">
						...
						<div className="sort btn-group" role="group" aria-label="Sort By">
						  	<button onClick={(e) => this.setState({	sortBy: e.target.value })} value="voteScore" type="button" className="btn btn-default voteScore">Vote</button>
						  	<button onClick={(e) => this.setState({	sortBy: e.target.value })} value="timestamp" type="button" className="btn btn-default timestamp">Time</button>
						</div>
					</div>
